...
class SendingValidator(Vertical):
    def __init__(self,text:str) -> None:
        ...
        self.frequency = SpinBox().minV(1).maxV(2147483647)
        self.min1 = SpinBox().minV(0).maxV(2147483647)
        self.max1 = SpinBox().minV(0).maxV(2147483647)
        self.min2 = SpinBox().minV(0).maxV(2147483647)
        self.max2 = SpinBox().minV(0).maxV(2147483647)
        self.add(GroupBox(
            Horizontal(
                (
                    ...
                    self.min1,
                    Text("max followers at", Text.Type.P3).enabled(False),
                    self.max1.enabled(False).set(2147483647),
                    Spacer()
                ),
                (
                    Text("min media count ", Text.Type.P3),
                    self.min2,
                    Text("max media count ", Text.Type.P3).enabled(False),
                    self.max2.enabled(False).set(2147483647),
                    Spacer()
                )
            ),
            text
    ...
    def __init__(self):
            ...
            Vertical(
                ...
                GroupBox(
                    (
                        GroupBox(
                            ...
                            [Button("Open").action(self.load_user_list), Spacer(), Button("Export to").enabled(False), ComboBox(["CSV", "JSON", "EXCEL", "PLAIN TEXT"]).id("q-exportType").enabled(False)],
                        ...
                        GroupBox(
                            ...
                            [Button("Open").action(self.load_accounts_list), Spacer(), Button("Export to").enabled(False), ComboBox(["CSV", "JSON", "EXCEL", "PLAIN TEXT"]).id("q-exportType").enabled(False)]
                            ),"Targets list"),
                ...
                GroupBox(
                    (
                        GroupBox(
                            [
                                RadioButton("iPhone").id("q-iphone").assign(self.deviceType).enabled(False),
                                RadioButton("Android").id("q-android").assign(self.deviceType).check(True)
                            ],
                            "Device Type"
                        ),
                        [Text("Device agent picking order", Text.Type.P3), ComboBox(
                            ["Default device agent", "Random device agent"]).id("q-deviceagentorder").enabled(False)]
                    ),
                ...
                GroupBox(
                    [
                        (
                            ...
                            FormField("Time before loading user").id(
                                "q-timebeforeloadinguser").enabled(False),
                            FormField("Time after loading user").id("q-timeafterloadinguser"),
                            FormField("Time before ending account task").id(
                                "q-timebeforeendingaccounttask").enabled(False),
                        ...
                        (
                            ...
                            FormField("Time before logout").id(
                                "q-timebeforelogout").enabled(False),
                ...
                GroupBox(
                    (
                        [Toggle().id("q-userDataToJSON").enabled(False), Text("Save user data to JSON", Text.Type.P3).enabled(False), Toggle(
                        ).id("q-userDataToCSV").enabled(False), Text("Save user data to CSV", Text.Type.P3).enabled(False)],
                        [Toggle().id("q-saveBlocksRecord").enabled(False), Text("Save blocks record", Text.Type.P3).enabled(False), Toggle(
                        ).id("q-saveBansRecord").enabled(False), Text("Save bans record", Text.Type.P3).enabled(False)],
                        ...
                        [Toggle().id("q-randomActions").enabled(False), Text("More random actions", Text.Type.P3).enabled(
                            False), Toggle().id("q-loginActions").check(True), Text("Login default actions", Text.Type.P3)],
                        [Toggle().id("q-downloadEmail").enabled(False),
                         Text("Save user email/data", Text.Type.P3).enabled(False)],
                    ),
        ...
        extract_section=(
            ...
            (
                ...
                GroupBox([Button("Select folder").action(self.select_extractor_save_folder), Label("None").id(
                    "e-folder"), CheckBox("Save extra data file").id("e-saveextra")], "Output file").expandMin(),
